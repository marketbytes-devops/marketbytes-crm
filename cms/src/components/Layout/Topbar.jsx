...
const Topbar = ({ toggleSidebar, isSidebarOpen, isAuthenticated, setIsAuthenticated, user, setWidgetSettings, setLanguage }) => {
  ...
  const [localWidgetSettings, setLocalWidgetSettings] = useState({
    ...
    dueProjects: true,
    projectStatusDiagram: true, // Add new widget
    projectsNearingDeadline: true, // Add new widget
  });
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-2">
        <button
          className="p-2 rounded-bl-lg transition-colors duration-200 bg-gray-100 hover:bg-black hover:text-white"
        ...
        <Dropdown triggerText="Widget Settings" icon={Settings}>
          <div className="space-y-2 w-[250px]">
            ...
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
                checked={localWidgetSettings.projectStatusDiagram}
                onChange={() => handleWidgetSettingsChange('projectStatusDiagram')}
                className="form-checkbox accent-black"
              />
              <span className="text-sm">Show Project Status Diagram</span>
            </label>
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
                checked={localWidgetSettings.projectsNearingDeadline}
                onChange={() => handleWidgetSettingsChange('projectsNearingDeadline')}
                className="form-checkbox accent-black"
              />
              <span className="text-sm">Show Projects Nearing Deadline</span>
            </label>
          </div>
        </Dropdown>
