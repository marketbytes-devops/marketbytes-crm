...
const RenewalReport = () => {
  ...
  // Mock data tailored for Email
  const mockData = [
    { id: 1, contactName: 'John Doe', companyName: 'Acme Corp', email: 'john@acme.com', renewalDate: '2025-10-01', amount: 30 },
    { id: 2, contactName: 'Jane Smith', companyName: 'Beta LLC', email: 'jane@beta.com', renewalDate: '2025-11-12', amount: 45 },
    { id: 3, contactName: 'Alice Johnson', companyName: 'Gamma Inc', email: 'alice@gamma.com', renewalDate: '2025-09-30', amount: 60 },
    { id: 4, contactName: 'Bob Lee', companyName: 'Delta Ltd', email: 'bob@delta.com', renewalDate: '2025-12-15', amount: 75 },
    { id: 5, contactName: 'Eva Green', companyName: 'Epsilon Co', email: 'eva@epsilon.com', renewalDate: '2025-10-20', amount: 40 },
  ];
  ...
  const downloadCSV = () => {
    const headers = ['ID,Contact Name,Company Name,Email,Email Renewal Date,Amount'];
    const csvRows = mockData.map(row => 
      `${row.id},"${row.contactName}","${row.companyName}","${row.email}","${row.renewalDate}",${row.amount}`
    );
    ...
    a.href = url;
    a.download = 'renewal_report_email.csv';
    a.click();
  ...
  const downloadExcel = () => {
    ...
    const ws = XLSX.utils.json_to_sheet(mockData, { header: ['id', 'contactName', 'companyName', 'email', 'renewalDate', 'amount'] });

    // Create a workbook and add the worksheet
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Email Renewal Report');

    // Generate and download the Excel file
    XLSX.writeFile(wb, 'renewal_report_email.xlsx');
  };
  ...
  const handlePrint = () => {
    ...
    const tableHTML = `
      <html>
        <head>
          <title>Email Renewal Report</title>
          <style>
        ...
        <table className="w-full text-left text-gray-600 border-collapse">
          <thead>
            <tr>
              ...
              <th className="p-2 border-b">EMAIL RENEWAL DATE</th>
