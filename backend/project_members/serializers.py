...
class EmployeeSerializer(serializers.ModelSerializer):
    ...
    employeeId = serializers.CharField(
        source="profile.employeeId", read_only=True, allow_null=True
    )
    ...
    dateOfBirth = serializers.DateField(
        source="profile.dateOfBirth", read_only=True, allow_null=True
    )
    joiningDate = serializers.DateField(
        source="profile.joiningDate", read_only=True, allow_null=True
    )
    probationPeriod = serializers.DateField(
        source="profile.probationPeriod", read_only=True, allow_null=True
    )
    slackUsername = serializers.CharField(
        source="profile.slackUsername", read_only=True, allow_null=True
    )
    exitDate = serializers.DateField(
        source="profile.exitDate", read_only=True, allow_null=True
    )
    gender = serializers.CharField(
        source="profile.gender", read_only=True, allow_null=True
    )
    address = serializers.CharField(
        source="profile.address", read_only=True, allow_null=True
    )
    skills = serializers.CharField(
        source="profile.skills", read_only=True, allow_null=True
    )
    mobile = serializers.CharField(
        source="profile.mobile", read_only=True, allow_null=True
    )
    hourlyRate = serializers.DecimalField(
        source="profile.hourlyRate",
        max_digits=10,
        decimal_places=2,
        read_only=True,
        allow_null=True,
    ...
    logIn = serializers.CharField(
        source="profile.logIn", read_only=True, allow_null=True
    )
    emailNotifications = serializers.CharField(
        source="profile.emailNotifications", read_only=True, allow_null=True
    )
    reportTo = serializers.CharField(
        source="profile.reportTo", read_only=True, allow_null=True
    )
    ...
    def get_designation(self, obj):
        try:
            profile = obj.profile
            designation_name = (
                profile.designation.designation_name if profile.designation else None
            )
...
class EmployeeCreateSerializer(serializers.ModelSerializer):
    ...
    logIn = serializers.ChoiceField(
        choices=[("Enable", "Enable"), ("Disable", "Disable")], required=False
    )
    emailNotifications = serializers.ChoiceField(
        choices=[("Enable", "Enable"), ("Disable", "Disable")], required=False
    )
    ...
    def validate(self, data):
        ...
        if User.objects.filter(email=email).exists():
            raise ValidationError(
                {"employeeEmail": "A user with this email already exists."}
            )
        ...
        if (
            ...
            raise ValidationError(
                {"employeeName": "A user with this name already exists."}
            )
    ...
    def create(self, validated_data):
        ...
        print(
            f"Created Profile for user {user.email}: designation={profile.designation}, department={profile.department}, employeeId={profile.employeeId}"
        )
        from works.models import Work
        from .models import ProjectMember

        default_project, created = Work.objects.get_or_create(
            workName="Unassigned Project", defaults={"workName": "Unassigned Project"}
        )
