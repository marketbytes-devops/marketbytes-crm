...
class TicketSerializer(serializers.ModelSerializer):
    ...
    agent = serializers.PrimaryKeyRelatedField(
        queryset=ProjectMember.objects.all(), allow_null=True, required=False
    )
    type = serializers.PrimaryKeyRelatedField(
        queryset=TicketType.objects.all(), allow_null=True, required=False
    )
    channel = serializers.PrimaryKeyRelatedField(
        queryset=Channel.objects.all(), allow_null=True, required=False
    )
    status = serializers.CharField(
        max_length=20, required=False, allow_null=True, default="Submit Open"
    )
    ...
    def get_requester(self, obj):
        assignee = self.get_assignee(obj)
        return (
            f"{assignee.get('name', 'Unassigned')} ({assignee.get('email', '')})"
            if assignee
            else "Unassigned"
        )
    ...
    def create(self, validated_data):
        ...
        if assignee_id and assignee_type:
            try:
                app_label = (
                    "clients" if assignee_type == "client" else "project_members"
                )
                content_type = ContentType.objects.get(
                    app_label=app_label, model=assignee_type
                )
                ...
                raise serializers.ValidationError(
                    {
                        "assignee_type": f"Invalid assignee type: {assignee_type}. Must be 'client' or 'projectmember'."
                    }
                )
